...
## Deployen (onder /demo van de hoofdsite)

De demo draait op `https://crowdstampxtra.netlify.app/demo/` (Vite `base: '/demo/'`, router-basename `/demo`). De `netlify.toml` in de **root** bouwt deze map en kopieert `dist` naar `/demo`; de SPA-redirect `/demo/* → /demo/index.html` staat daar ook. `/demo` staat in `.gitignore`.
